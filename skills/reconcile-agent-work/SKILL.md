...
Never extract, replay, or share browser cookies, auth headers, refresh tokens, session tokens, or keychain records. Moving to the next account the operator owns, on that account's own login, is the supported response to a reported limit; forcing requests past a limit is not.
