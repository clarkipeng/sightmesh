...
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "recovery-smoke: would verify agent-deck service and lease recovery in a temporary HOME"
  exit 0
fi
...
printf '#!/usr/bin/env bash\nexit 0\n' >"${tmpdir}/bin/agent-deck"
chmod +x "${tmpdir}/bin/cdesktop" "${tmpdir}/bin/agent-deck"
