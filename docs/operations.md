...
`sightmesh --json overview` uses the fixed privacy-safe fleet projection and does not copy transcripts. Model comes from the native execution action. Its selected cdesktop provider ID is used only to join the native provider kind; it is not reported as a SightMesh subscription account. Token usage and context limits appear only when cdesktop supplies a normalized `token_usage_info` entry for the selected process, with that provenance recorded. Account, quota, monetary cost, and any other unavailable fact remain `null`; SightMesh does not infer them.
