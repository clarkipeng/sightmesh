...
The subscription row's unique canonical output root is the lease. `sightmesh run subscribe` prepares one fresh absent or empty root and atomically records the subscription and lease in one insert; `sightmesh run bind` verifies the runner's `(pid, process_start)` fingerprint after launch. Terminal or lost evidence sets the logical lease-release time, while the historical root identity stays reserved so append-only evidence can never be reused for another run. SightMesh never launches, waits for, kills, retries, or interprets the run.
