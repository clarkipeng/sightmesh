...
def test_history_write_rolls_back_with_projection(tmp_path):
    ...
    with pytest.raises(RuntimeError):
        with store.connect() as conn:
            ...
            conn.execute(
                "INSERT INTO task_effects(task_id,epoch,request_hash,state,owner_instance,lease_expires_at,created_at,updated_at) VALUES('x',1,'h','reserved','o',1,1,1)"
            )
            row = conn.execute(
                "SELECT * FROM task_effects WHERE task_id='x'"
            ).fetchone()
    ...
    with store.connect() as conn:
        assert (
            conn.execute(
                "SELECT count(*) FROM task_effects WHERE task_id='x'"
            ).fetchone()[0]
            == 0
        )
...
def test_real_effect_reserve_rolls_back_when_its_history_append_fails(
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: (_ for _ in ()).throw(
            sqlite3.DatabaseError("history failed")
        ),
    )
    with pytest.raises(Exception):
        journal.reserve("task", 1, request_hash({"x": 1}), "owner")
...
def test_effect_side_doors_each_leave_an_observed_history_entry(tmp_path):
    ...
    class Client:
        def stop_workspace(self, _workspace):
            return None
    ...
    with store.connect() as conn:
        task_causes = [
            row["cause"] for row in history.task_history(conn, "task", entity="effect")
        ]
        expired_causes = [
            row["cause"]
            for row in history.task_history(conn, "expired", entity="effect")
        ]
    assert {"reservation-takeover", "cleanup-workspace", "cleanup-acknowledged"} <= set(
        task_causes
    )
...
def test_real_old_shape_upgrade_baselines_migrated_task_and_effect(tmp_path):
    ...
    assert {
        ...
        ("task", "baseline", 1, "observed-baseline"),
        ("effect", "baseline", 1, "observed-baseline"),
    }
