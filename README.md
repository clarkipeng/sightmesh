...
The managed bridge checks every active spawned child for a completed event-snapshot silence interval, including `--no-bridge` children. Set `SIGHTMESH_STALL_THRESHOLD_MINUTES` to a whole number from 1 to 1440 before `sightmesh service install` to configure the managed LaunchAgent; rerun install after changing it. Invalid values safely use the 30-minute default.
